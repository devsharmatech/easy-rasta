...
export default function ToiletReportsPage() {
    ...
    const fetchReports = async () => {
        try {
            setLoading(true)
            const res = await fetch('/api/admin/toilet-reports', { headers })
    ...
    const filtered = reports.filter(r => {
        ...
        const matchSearch = scoutName.includes(searchTerm.toLowerCase()) || locationName.includes(searchTerm.toLowerCase())
    ...
    const handleAction = async (report, action) => {
        ...
        try {
            const res = await fetch('/api/admin/toilet-reports', {
                method: 'PATCH',
                headers,
    ...
    const getHygieneColor = (level) => {
        if (level === 'clean') return 'text-green-600 bg-green-50'
        if (level === 'average') return 'text-amber-600 bg-amber-50'
        if (level === 'dirty') return 'text-red-600 bg-red-50'
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">Toilet Reports</h2>
                    <p className="text-muted-foreground">Manage community hygiene reports & verifications.</p>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
                <div className="relative flex-1 min-w-[200px] max-w-sm">
                    ...
                    <Input placeholder="Search user or location name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
                </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-gray-50/50">
                                ...
                                <TableHead className="font-semibold">Hygiene</TableHead>
                            ...
                            {filtered.length > 0 ? filtered.map((report) => (
                                <TableRow key={report.id} className="hover:bg-gray-50/50 transition-colors">
                                    <TableCell>
                                        <div className="h-12 w-12 rounded overflow-hidden bg-gray-100 flex items-center justify-center cursor-pointer" onClick={() => setDetailReport(report)}>
                                            {report.photo_url ? (
                                                <Image src={report.photo_url} alt="Toilet photo" width={48} height={48} className="object-cover h-full w-full" />
                                            ) : (
                                    ...
                                    <TableCell>
                                        <p className="text-sm font-medium w-48 truncate">{report.location_name || 'Unknown Location'}</p>
                                        <p className="text-[10px] text-gray-400 font-mono mt-0.5">{report.latitude}, {report.longitude}</p>
                                        {report.is_paid ? (
                                            <span className="inline-block mt-1 px-1.5 py-0.5 bg-red-50 text-red-600 border border-red-100 rounded text-[9px] font-bold">PAID: ₹{report.price || '?'}</span>
                                        ) : (
                                            <span className="inline-block mt-1 px-1.5 py-0.5 bg-green-50 text-green-600 border border-green-100 rounded text-[9px] font-bold">FREE</span>
                                        )}
                                    </TableCell>
                                    <TableCell>
                                        <span className={`px-2 py-0.5 rounded text-xs font-semibold capitalize ${getHygieneColor(report.hygiene_level)}`}>
                                            {report.hygiene_level || 'Unknown'}
                                        </span>
                                    </TableCell>
                                    <TableCell>
                                ...
                                <TableRow>
                                    <TableCell colSpan={7} className="text-center py-12 text-muted-foreground">
                                        ...
                                        <p className="font-medium">No toilet reports found</p>
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
                ...
                <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            <ShieldCheck className="h-5 w-5 text-blue-600" /> Toilet Report Details
                        </DialogTitle>
                    </DialogHeader>
                    {detailReport && (
                        <div className="space-y-4">
                            ...
                            {detailReport.photo_url && (
                                <div className="w-full h-64 bg-gray-100 rounded-lg overflow-hidden relative">
                                    <Image src={detailReport.photo_url} alt="Toilet photo" fill className="object-cover" />
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="bg-gray-50 rounded-lg p-3">
                                    <p className="text-xs text-muted-foreground mb-1">Hygiene Level</p>
                                    <span className={`px-2 py-1 rounded text-sm font-semibold capitalize ${getHygieneColor(detailReport.hygiene_level)}`}>
                                        {detailReport.hygiene_level || 'Unknown'}
                                    </span>
                                </div>
                                <div className="bg-gray-50 rounded-lg p-3">
                                ...
                                <div className="bg-gray-50 rounded-lg p-3 col-span-2 flex justify-between items-center">
                                    <div>
                                        <p className="text-xs text-muted-foreground mb-1">Location Name</p>
                                        <p className="font-medium text-sm">{detailReport.location_name || 'N/A'}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-xs text-muted-foreground mb-1">Access Type</p>
                                        {detailReport.is_paid ? (
                                            <span className="inline-block px-2 py-1 bg-red-100 text-red-700 rounded text-xs font-bold">Paid Toilet (₹{detailReport.price || '?'})</span>
                                        ) : (
                                            <span className="inline-block px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-bold">Free Access</span>
                                        )}
                                    </div>
                                </div>
                            </div>
